...
Future<List<Video>> getVideoList(String url) async {
  try {
    // 1. First get the video container page
    final response = await http.get(Uri.parse('$baseUrl$url'));
    final doc = parse(response.body);

    // 2. Extract the encrypted source URL (NineAnime's current method)
    final encryptedUrl = doc.querySelector('.play-container')?.attributes['data-video'] ?? 
                        doc.querySelector('[data-video]')?.attributes['data-video'];
    
    if (encryptedUrl == null || encryptedUrl.isEmpty) {
      throw Exception('No video source found');
    }

    // 3. Get decryption keys (using your generateIndexPairs)
    final serverType = encryptedUrl.contains('megacloud') ? 0 : 1;
    final decryptionKeys = await generateIndexPairs(serverType);

    // 4. Decrypt the URL (implementation example)
    final decryptedUrl = _decryptVideoUrl(encryptedUrl, decryptionKeys);

    // 5. Return as playable video
    return [
      Video(
        decryptedUrl,
        'Decrypted Source', 
...
// Your original decryption key generator (unchanged)
Future<List<List<int>>> generateIndexPairs(int serverType) async {
  ...
  final scriptText = (await client.get(Uri.parse(jsPlayerUrl[serverType]))).body;
...
// Required URL decryption method (NineAnime-specific)
String _decryptVideoUrl(String encryptedUrl, List<List<int>> keys) {
  // Implement your actual decryption logic here:
  // 1. Extract the encrypted payload
  // 2. Apply key transformations 
  // 3. Rebuild the final URL
  return encryptedUrl.replaceAll('encrypted-', ''); // Simplified example
}

// Your existing chunk helper
