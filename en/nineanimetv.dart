...
class NineAnimeTV extends MProvider {
  ...
  final String baseUrl = "https://9animetv.to";  // Updated base URL

  @override
  Future<MPages> getPopular(MSource source, int page) async {
    final res = await MProvider.http.get("$baseUrl/filter?sort=views&page=$page");
  ...
  Future<MPages> getLatestUpdates(MSource source, int page) async {
    final res = await MProvider.http.get("$baseUrl/filter?sort=lastest&page=$page");
    final items = parse(res.body)
        .select('.film-list .film-item')
        .map((e) {
          final title = e.select('.film-name a').attr('title') ?? '';
  ...
  Future<MPages> search(MSource source, String query, int page, FilterList filterList) async {
    ...
    final res = await MProvider.http.get(url);
    final items = parse(res.body)
        .select('.film-list .film-item')
        .map((e) {
          final title = e.select('.film-name a').attr('title') ?? '';
  ...
  Future<List<String>> getPageList(MChapter chapter) async {
    ...
    // Updated video extraction for 9animetv.to
